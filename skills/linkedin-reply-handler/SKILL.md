...
2. **Fetch the full comment tree.** Call `lib.ApifyClient.fetch_post_comments(post_id=<post_urn or resolved canonical id>, max_items=100)`. If `APIFY_TOKEN` is not set, ask the user to paste the comment list (name + text per comment is enough; nested replies noted as such).
